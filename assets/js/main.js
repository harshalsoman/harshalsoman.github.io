...
document.addEventListener("DOMContentLoaded", function() {
    const titles = ["Aspiring Data Scientist", "Software Engineer", "Machine Learning Engineer"];
    let currentIndex = 0;
    const jobTitleElement = document.getElementById('jobTitle');
    
    function updateTitle() {
        jobTitleElement.classList.add('fade');
        setTimeout(() => {
            jobTitleElement.textContent = titles[currentIndex];
            jobTitleElement.classList.remove('fade');
            currentIndex = (currentIndex + 1) % titles.length;
        }, 500); // Match the CSS transition duration
    }
    updateTitle();
    setInterval(updateTitle, 1000); // Change title every 3.5 seconds to include fade time
});